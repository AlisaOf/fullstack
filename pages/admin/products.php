...
$query = "SELECT * p.id, p.name, p.price, p.description, p.picture, c.name as category
        FROM poducts as p
        JOIN categories as c ON p.categori_id = c.id";
$res = $pdo->query($query);
...
<form method="POST" enctype="multipart/form-data" action="../actions/admin/createProducts.php">
    <input value="<?= $_SESSION['lastProductCreate']['name'] ?? ''?>" class="form-control md-2" name='name' placeholder="Наименование продукта">
    ...
    <select class="form-control mb-2" name='category_id'>
        ...
        <?php
             $iastCategoryId = $_SESSION['lastProductCreate']['category_id'];
             if ($lastCategoryId) {
        ...
        foreach ($products as $product) {
            ...
            <tr>
              ...
              <td class='text-center'>
                 <img height='100' scr='{'$path'}{$product['picture']}'>
              </td>
            </tr>
            ";
        }
